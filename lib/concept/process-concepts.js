...
const PROPERTIES_WITH_CHILD_CONCEPTS = ['values', 'concepts', 'data', 'enable-data', 'components', 'unit', 'default', 'graph'];
...
function existingConceptInfo(id, value, targetType, level = 0) {
  const space = _.repeat(' ', level * 2);
  return chalk`Existing:${space} {yellow ${id}} -> {yellow ${value}} {gray (${targetType})}`;
}
    ...
    return _.cloneDeepWith(target, (value, key) => {
      switch (true) {
        ...
        case _.isString(value) && value.startsWith(targetType): {
          ...
          if (conceptRegistry[childConceptName]) {
            log.info(existingConceptInfo(childConceptName, value, conceptRegistry[childConceptName].type, level));
            return childConceptName;
          }

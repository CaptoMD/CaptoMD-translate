...
function extractInline(concepts, { verbose = false } = {}) {
  return _.reduce(
    concepts,
    (results, concept, id) => Object.assign(results, extractInlineConcepts(concept, id, { verbose })),
    {}
  );
}

function extractComplexConcept(concepts, { verbose = false } = {}) {
  ...
  return _.map(complexConceptIds, rootId => {
...
module.exports = function extractConcepts(concepts, { verbose = false, inline = true, complex = true } = {}) {
  const complexConceptChildren = complex ? extractComplexConcept(concepts, { verbose }) : [];
  const inlineValues = inline ? extractInline(concepts, { verbose }) : {};

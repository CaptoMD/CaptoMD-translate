...
async function readConceptFile(rootPath, fileName, { verbose = false } = {}) {
...
async function readConcepts(rootPath, { verbose = true, inline = true, complex = true } = {}) {
  if (verbose) {
    console.log(chalk`Reading: {bgCyan.black ${rootPath}}`);
  }
  const files = await readdir(path.resolve(rootPath), 'utf8');
  const concepts = extractConcepts(await readConceptFile(rootPath, files, { verbose }), { verbose, inline, complex });
  if (verbose) {

...
function processRow(row, concept, languages) {
  ...
  if (row['placeholder-label']) {
    ...
    if (hasPlaceholder) {
      log.info(
        chalk`Concept {red ${conceptId}} {gray (${concept.type})} cannot specify {bold placeholder} and {bold placeholder-label}`
      );
    }
  }
  ...
  if (!hasLabels) {
    if (isNone) {
      ...
      log.info(
        chalk`Concept {yellow ${conceptId}} {gray (${concept.type})} has no labels will use labels of {cyan ${fallBack}}`
      );
    } else {
...
async function processWorksheet(sheet, concepts, languages) {
  const rows = await getRows(sheet, { offset: 3 });
...
module.exports = async function extractConceptLabels(conceptsSheet, conceptValuesSheet, concepts, languages) {
  ...
  return _.merge({ config: { ...conceptsConfig, ...valuesConfig } }, conceptsTranslations, valuesTranslations);
};
